...
export default function Home() {
  ...
  // Set page title
  useEffect(() => {
    document.title = "Home | Anjie Zhou";
  }, []);
  ...
  useEffect(() => {
    ...
    // Start bounce animation after both header parts are visible
    const timer4 = setTimeout(() => setShowBounce(true), 1800);
    return () => {
      clearTimeout(timer1);
      clearTimeout(timer2);
      clearTimeout(timer3);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background-light dark:bg-background-dark text-foreground-light dark:text-foreground-dark transition-colors">
      ...
      <main className="flex flex-1 flex-col items-center w-full">
        ...
        <div ref={heroRef} className="w-full relative" style={{ height: 'calc(100vh - 64px)' }}>
            ...
            <h1 className="text-5xl md:text-7xl font-extrabold text-center text-foreground-light dark:text-foreground-dark drop-shadow-lg select-none">
              {Array.from("Welcome... To My World.").map((char, i) => (
                <span
                  ...
                  } ${showBounce ? 'animate-bounce-once' : ''}`}
                  style={showBounce ? {
