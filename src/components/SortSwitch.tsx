...
export default function SortSwitch({ value, onChange, className = "" }: SortSwitchProps) {
  ...
  // Track colors
  const trackOff = isDarkMode ? "#23201c" : "#ece7d5";
  const trackOn = isDarkMode ? "#5a554f" : "#cccab9";
  // Thumb colors: always off-white (same as light mode) in both modes
  const thumbColor = "#ece7d5";
  ...
  const ringColor = '0 0 0 4px rgba(0,0,0,0.1)';
  ...
  const knobStyle = {
    boxShadow: isActive
      ? `0 1px 4px rgba(0,0,0,0.10), ${ringColor}`
